...
namespace SistemasVentas.DAL
{
    public class ProductoDal
    {
        ...
        public DataTable ProductosDal()
        {
            string consulta = "SELECT COUNT(*) AS TOTAL_PRODUCTOS\r\nFROM PRODUCTO;\r\n";
            return conexion.EjecutarDataTabla(consulta, "fsadf");

        }
        public DataTable ProductosCincoDal()
        {
            string consulta = "SELECT PRODUCTO.NOMBRE, DETALLEING.FECHAVENC\r\nFROM PRODUCTO\r\nINNER JOIN DETALLEING ON PRODUCTO.IDPRODUCTO = DETALLEING.IDPRODUCTO\r\nWHERE DETALLEING.FECHAVENC BETWEEN GETDATE() AND DATEADD(DAY, 7, GETDATE());";
            return conexion.EjecutarDataTabla(consulta, "fsadf");

        }
        public DataTable ProductosSeisDal()
        {
            string consulta = "SELECT V.IDVENDEDOR, P.NOMBRE AS Nombre_Vendedor, SUM(DV.CANTIDAD) AS Cantidad_Productos_Vendidos\r\nFROM PERSONA P\r\nINNER JOIN USUARIO U ON P.IDPERSONA = U.IDPERSONA\r\nINNER JOIN VENTA V ON U.IDUSUARIO = V.IDVENDEDOR\r\nINNER JOIN DETALLEVENTA DV ON V.IDVENTA = DV.IDVENTA\r\nGROUP BY V.IDVENDEDOR, P.NOMBRE;";
            return conexion.EjecutarDataTabla(consulta, "fsadf");
